...
def densify(boundsArr, dens=4):
    ...
    '''
    denseBounds = np.zeros(((boundsArr.shape[0]) * dens, 2), dtype=boundsArr.dtype)
...
def crosses_dateline(fpath):
    ...
    with rio.open(fpath) as src:
        ...
        unprojectedBounds = warp.transform(src.crs, {'init': 'epsg:4326'}, denseBounds[:, 0], denseBounds[:, 1])
